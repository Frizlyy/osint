...
# -*- coding: utf-8 -*-
# creator : r!xz
# warning : dilarng diperjual belikan tools ini!
# hak cipta berlaku.
...
def footer():
    ...
    sig = [
        f"{P}  -r¡xz-",
        f"{P}  Follow my channel whatsapp",
        f"{P}  https://whatsapp.com/channel/0029Vb3yY4aBVJkuokDlEc0Y",	
        f"{C}  TOOLS INI MUNGKIN TIDAK AKURAT! KARENA MENGGUNAKAN SYSTEM DORKING",
          ]
    for line in sig: print(center_text(line))
    print(center_text(f"{P2}r¡xz osint tools | version 1.0"))
...
def f_dork_manager():
    banner()
    print(center_text(f"{P2}:: GOOGLE DORK GEN ::{W}"))
    print("\n")
    print(center_text(f"{P}[1] Load from File (dork.txt){W}"))
    print(center_text(f"{C}[2] Generate for Target Domain{W}"))
    ...
    if choice == '1':
        ...
        # MODE GENERATOR
        print(f"\n{Y}[ GENERATOR MODE ]{W}")
        domain = input(f"{P}Target Domain (ex: site.com): {W}").strip()
        if not domain: return
        
        print(f"\n{G}[*] Generating Dorks for {domain}...{W}")
        
        # Logic Generator Tambahin sendiri atau gunakan pilihan 1 copas kesini!
        patterns = [
            'inurl:admin', 'inurl:login', 'inurl:cpanel', 'inurl:dashboard',
            'intitle:"index of"', 'intext:"sql syntax near"', 'ext:xml', 
            'ext:conf', 'ext:log', 'ext:sql', 'inurl:wp-admin', 
            'inurl:upload', 'inurl:php?id=', 'filetype:xls', 
            'filetype:env', 'intext:"password"', 'inurl:shell'
        ]
        
        generated = []
        for p in patterns:
            dork = f"site:{domain} {p}"
            generated.append(dork)
            print(f"{C}-> {W}{dork}")
            
        print(f"\n{G}[SUCCESS] Generated {len(generated)} Dorks.{W}")
        save = input(f"{P}Save to file? (y/n): {W}")
        if save.lower() == 'y':
            fname = f"dork_{domain}.txt"
            with open(fname, "w") as f:
                for d in generated: f.write(d+"\n")
            print(f"{Y}Saved to {fname}{W}")
...
def f_subdomain():
    ...
    try:
        ...
        try:
            ...
            print(f"{R}[!] No JSON response (Limit/Block){W}")
...
def main():
    while True:
        banner()
        draw_dual_box("1. Dork Manager", "2. Subdomain")
